...
- **Retries failures on its own.** A sync that throws (IMAP server hiccup, Proton Bridge restarting, network not up yet) is retried by Messenger with exponential backoff — 1, 2, 4, 8 and 16 minutes (`config/packages/messenger.yaml`), enough to outlive most transient outages; only persistent failures (e.g. a bad password) land in the `failed` transport for manual review. A sync that is *killed* mid-run (worker crash, OOM, deploy) is re-queued by `WorkerStartupListener` when the sync worker comes back, instead of waiting for the doctrine transport's one-hour redelivery or a manual "Sync now". Both paths lean on the cursor: a retry resumes, it never redoes work.
